...
namespace anda\core\widgets\cropimageupload;
...
class CropImageUploadBehavior extends UploadBehavior
{
	...
	protected function createCrop()
	{
		...
		if(is_array($this->resize)){
			if(count($this->resize) > 0){
				$width = intval($this->resize[0]);
				$height = (isset($this->resize[1])) ? intval($this->resize[1]) : intval($this->resize[0]);
				$image->crop(new Point($crop[0], $crop[1]), new Box($crop[2]-$crop[0], $crop[3]-$crop[1]))->thumbnail(new Box($width, $height))->save($save_path);
			}else{
				$image->crop(new Point($crop[0], $crop[1]), new Box($crop[2]-$crop[0], $crop[3]-$crop[1]))->save($save_path);
			}
		}else{
